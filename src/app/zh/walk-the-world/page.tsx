  ...
  if (success) {
    return (
      <div className="rounded-2xl border border-blue-primary/20 bg-blue-primary/5 px-8 py-12 text-center">
        ...
        <h3 className="mb-2 font-heading text-2xl font-semibold text-foreground">报名成功！</h3>
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        ...
        <select value={ticketType} onChange={(e) => setTicketType(e.target.value)} className={ic}>
          ...
          <option value="couple">双人票</option>
          <option value="group">团体票（3-10人）</option>
        </select>
      </div>
      {ticketType === "group" && <div><label className={lc}>团体人数</label><input type="number" min={3} max={10} placeholder="3-10" className={ic} /></div>}
      <div><label className={lc}>紧急联系人（选填）</label><input placeholder="姓名及联系电话" className={ic} /></div>
...
const highlights = [
  { icon: <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>, title: "风景路线", description: "穿行于精心甄选的户外胜地，感受自然之美与蓬勃生命力。" },
  { icon: <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>, title: "社区连接", description: "与志同道合的人相遇——他们同样热爱运动、健康和有意义的社交体验。" },
  { icon: <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>, title: "健康活动", description: "体验引导式的身心健康时刻，包括正念呼吸、拉伸和团体活动。" },
  { icon: <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" /></svg>, title: "清新能量", description: "在晨光中启程——研究证明，日光下的运动能显著提升情绪、精力和长期健康。" },
];

const schedule = [
  { time: "7:00 AM", activity: "签到与欢迎", detail: "签到、领取欢迎礼包、集体热身拉伸" },
  { time: "7:30 AM", activity: "行走启程", detail: "沿风景路线引导行走，途经多个健康体验站" },
  { time: "9:00 AM", activity: "健康驿站", detail: "正念呼吸、团体活动、补充能量" },
  { time: "10:00 AM", activity: "抵达与庆祝", detail: "社区聚会、合影留念、行走后茶歇" },
];

/* ──────────────────────────────────────────────
   Walk The World Page (Chinese)
   ────────────────────────────────────────────── */

export default function WalkTheWorldPageZh() {
  return (
    <>
      <NavbarZh />
      <main className="flex-1">
        ...
        <section className="relative flex min-h-[80vh] items-center justify-center overflow-hidden">
          ...
          <div className="relative z-10 mx-auto max-w-7xl px-6 pb-20 pt-32 text-center lg:px-8">
            <p className="mb-6 inline-block rounded-full border border-white/10 bg-white/10 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.2em] text-gold-light backdrop-blur-sm">天行健 · 唤醒</p>
            <h1 className="mb-6 font-heading text-4xl font-bold text-white sm:text-5xl md:text-6xl lg:text-7xl">Walk The World · 走世界</h1>
            <p className="mx-auto mb-12 max-w-2xl text-lg text-white/70 md:text-xl">一场户外行走体验，将运动、自然与社区融为一体。在晨光中出发，感受清新能量与有意识的连接。</p>
            <CountdownTimer targetDate="2026-06-15T07:00:00+08:00" label="活动倒计时" />
          </div>
        </section>
        ...
        <section className="bg-background-soft py-24 lg:py-32">
          <div className="mx-auto max-w-7xl px-6 lg:px-8">
            <div className="mb-16 text-center">
              <p className="mb-4 text-xs font-semibold uppercase tracking-[0.2em] text-blue-primary">体验亮点</p>
              <h2 className="font-heading text-3xl font-bold text-foreground md:text-4xl">不止是一次行走</h2>
              <p className="mx-auto mt-4 max-w-lg text-foreground-muted">每一个环节都精心设计，唤醒你的身体、心灵与社交连接。</p>
            </div>
        ...
        <section className="py-24 lg:py-32">
          <div className="mx-auto max-w-3xl px-6 lg:px-8">
            <div className="mb-16 text-center">
              <p className="mb-4 text-xs font-semibold uppercase tracking-[0.2em] text-blue-primary">活动流程</p>
              <h2 className="font-heading text-3xl font-bold text-foreground md:text-4xl">你的晨间旅程</h2>
            </div>
          ...
          <div className="mx-auto max-w-7xl px-6 lg:px-8">
            <div className="grid gap-6 md:grid-cols-3">
              <div className="rounded-2xl border border-foreground/5 bg-white p-8 text-center shadow-elevated">
                ...
                <p className="font-heading text-2xl font-bold text-foreground">2026年6月15日</p>
                <p className="mt-1 text-sm text-foreground-muted">周六清晨</p>
              </div>
              <div className="rounded-2xl border border-foreground/5 bg-white p-8 text-center shadow-elevated">
                <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-foreground-muted">地点</p>
                <p className="font-heading text-2xl font-bold text-foreground">滨海湾花园</p>
                <p className="mt-1 text-sm text-foreground-muted">新加坡</p>
              </div>
              <div className="rounded-2xl border border-foreground/5 bg-white p-8 text-center shadow-elevated">
                <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-foreground-muted">路程</p>
                <p className="font-heading text-2xl font-bold text-foreground">5 公里</p>
                <p className="mt-1 text-sm text-foreground-muted">风景路线</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="register" className="py-24 lg:py-32">
          <div className="mx-auto max-w-xl px-6 lg:px-8">
            <div className="mb-12 text-center">
              ...
              <h2 className="mb-4 font-heading text-3xl font-bold text-foreground md:text-4xl">加入 Walk The World</h2>
              <p className="text-foreground-muted">立即锁定你的名额。名额有限，先到先得。</p>
            </div>
        ...
        <section className="bg-background-soft py-24 lg:py-32">
          <div className="mx-auto max-w-7xl px-6 lg:px-8">
            <div className="mb-12 text-center">
              <p className="mb-4 text-xs font-semibold uppercase tracking-[0.2em] text-blue-primary">精彩回顾</p>
              <h2 className="font-heading text-3xl font-bold text-foreground md:text-4xl">往期体验</h2>
            </div>
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="relative aspect-square overflow-hidden rounded-2xl bg-gradient-to-br from-blue-primary/15 to-[#2E8B57]/10">
                  <div className="absolute inset-0 flex items-center justify-center"><p className="text-sm text-foreground-muted/40">照片 {i}</p></div>
                </div>
              ))}
            </div>
          </div>
